...
public class MinimapFloor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        s_Exposed = GameObject.FindGameObjectWithTag("Holder").GetComponent<MinimapIconHolder>().Floor;
    ...
    void Update()
    {
        if (Time.frameCount % 10 == 0)  // Check every 10th frame so as to not lag the performance too much
        {
            ...
            if (GeometryUtility.TestPlanesAABB(planes, gameObject.GetComponent<BoxCollider2D>().bounds))
            {
                ...
                Destroy(this); // delete the script since we won't be needing it anymore - to reduce Load
            }
        }
    }
}
